...
var delaytime =[8500,8500,4700,8500,5500,8500]
function testing(){
	var index = piece1.getAttribute("xlink:href").slice(8,9) -1
	// console.log(index)
	// console.log(index)
	var audioi=index+1;
	var audiogif = document.getElementById("audio"+audioi)
	if(!isRight.includes(false)){
		isLetter = true;
		$("#p_gif").css("opacity","1")
		$("#puzzle").delay(1500).fadeOut("slow");
		$("#p_gif").css("animation-name","puzzleani")
		console.log(audiogif)
		setTimeout(function(){audiogif.play()},2000)
		setTimeout(function(){
			letterOn();
		},delaytime[index])
...
function letterOff(){
	...
	$("#blackbg").css("pointer-events","none")
	setTimeout(function(){
		$("#p_tip").css("z-index","0")
...
var poster_back = document.getElementById("poster_back");
poster_back.onclick = function(){
	backtoGallery();
	$(".page4").fadeOut("slow");
	$(".page4").css("pointer-events","none")
	save.innerHTML = " ";
...
var postertest = document.getElementById("saveposter")
console.log(postertest.width)
// console.log(canvash)
function savePoster(){
	var offsetLeft = poster.getBoundingClientRect().x;
	console.log(document.body.scrollWidth)
    html2canvas(postertest,{
		// useCORS: true,
		...
		// saveImg.css()
        saveImg.id = 'saveImg';
        saveImg.src= imgURL;//imgUrl是html2canvas返回的截图的base64码
        save.appendChild(saveImg);//将生成的截图放到页面中
    })
}
